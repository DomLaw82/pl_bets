...
def get_rolling_stat(df: pd.DataFrame, stat: str, n: int) -> pd.DataFrame:
    ...
    try:
        ...
        raise Exception(f"An error occurred during getting rolling stat: line {e.__traceback__.tb_lineno} : {str(e)}")
...
def get_rolling_goal_difference_at_location(df: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    try:
        ...
        raise Exception(f"An error occurred during getting rolling goal difference at location: line {e.__traceback__.tb_lineno} : {str(e)}")
...
def add_historic_head_to_head_results(data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        raise Exception(f"An error occurred during adding win prediction - historic head to head results: line {e.__traceback__.tb_lineno} : {str(e)}")
...
def get_days_since_last_league_game(data: pd.DataFrame, team_id: str) -> pd.DataFrame:
    ...
    try:
        ...
        Exception(f"An error occurred during win prediction - days since last league game: line {e.__traceback__.tb_lineno} : {str(e)}")
...
def run_data_prep(sql_connection: SQLConnection, features: list, fixtures: pd.DataFrame = pd.DataFrame()) -> pd.DataFrame:
    ...
    try:
        ...
        raise Exception(f"An error occurred during win prediction data preparation: line {e.__traceback__.tb_lineno} : {str(e)}")