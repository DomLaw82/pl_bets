...
def create_prediction_dataset(home_team_id: str, home_team_squad: list, away_team_id: str, away_team_squad: list) -> pd.DataFrame:
	...
	home_team_squad_ids = db.get_list(f"""
		...
		WHERE
			...
			{"AND player_team.player_id IN ('"+"','".join(home_team_squad) + "')" if len(home_team_squad) > 0 else ''}
	""")
	away_team_squad_ids = db.get_list(f"""
		...
		WHERE
			...
			{"AND player_team.player_id IN ('"+"','".join(away_team_squad) + "')" if len(away_team_squad) > 0 else ''}
	""")
	...
	df["head_to_head_goal_difference"] = df.apply(lambda row: get_last_five_head_to_head_matches(row["home_team_id"], row["away_team_id"]), axis=1) 
