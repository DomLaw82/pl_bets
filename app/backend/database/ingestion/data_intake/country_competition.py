...
def save_to_database(db_connector, df: pd.DataFrame, team_name: str) -> None:
	df.to_sql(team_name, db_connector.conn, if_exists="append", index=False)

def country_competition_main(db_connector):
	dfs = clean_country_competition_data()
	tables = ["competition", "country"]
	
	for idx, df in enumerate(dfs):
		save_to_database(db_connector, df, tables[idx])