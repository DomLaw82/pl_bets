...
def refresh_game_data():
    try:
        ...
        download_csv_for_all_games_in_a_season(current_season, game_data_download_root, game_data_save_root)
        two_digit_season = "".join([year[-2:] for year in current_season.split("-")])
...
def refresh_schedule_data():
    try:
        ...
        download_csv_for_all_fixtures_in_a_season(current_season, schedule_data_download_root, schedule_data_save_root)

        season_schedule_file_path = os.path.join(schedule_data_save_root, f"epl_{current_season.split('-')[0]}-{current_season.split('-')[1][-2:]}")
