...
export default function Matches(props) {
    ...
    async function getMatchFacts(date, homeTeamName, awayTeamName) {
        ...
        const response = await fetch(`${process.env.DATA_API_ROOT}/matches/match-facts?date=${formattedDate}&home_team=${homeTeamName.replace(/&/g, "%26")}&away_team=${awayTeamName.replace(/&/g, "%26")}`, {
            headers: {
    ...
    async function getMatches(season) {
        const response = await fetch(`${process.env.DATA_API_ROOT}/matches/season/${season}`, {
            headers: {
                'Access-Control-Allow-Origin': '*'
            }
        });
